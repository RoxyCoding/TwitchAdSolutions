...
### Changed
- **Client-side ad requests are now blocked** — every request to `edge.ads.twitch.tv` (Twitch's CSAI ad server) is failed the way a network filter would: `fetch` rejects with a `TypeError`, XHR goes DONE / status 0 and fires `readystatechange` → `error` → `loadend` asynchronously. The ad SDK therefore never receives a creative, so the separate video-ad slot (#249) is never rendered — nothing to hide or fast-forward. This is the state Pi-hole / uBO network-filter users have always been in; the m3u8 side (SSAI markers, backup swap) is unaffected. Both transports are still counted in the `CSAI ad request (…, blocked)` log. Detection trade-off accepted by the fork owner. Opt-out: `twitchAdSolutions_blockCsaiAdRequests=false` (requests are then logged and allowed through, and the hide / fast-forward guard handles the slot as before). vaft only
- **Separate video-ad slot hidden before first paint** — fallback for when a creative does get through (opt-out above, or a transport the hooks don't see): a `<style>` installed at document-start hides the ad `<video>`, `.outstream-controls`, and the collapsible slot container (`div[style*="transition: max-height"]:has(...)`), so the slot never flashes while waiting for the next 600ms guard tick. Three separate rules; a browser without `:has()` only loses the container one. vaft only (#249)

## v68.5.12 (2026-09-13) — RoxyCoding fork
...
### Fixed
- **The SDA hide is now re-asserted on every tick, and its marker actually dedupes** — `dataset.tasHidden` was assigned `''`, which `dataset` reads back as the empty string and is therefore falsy, so the `if (!...dataset.tasHidden)` dedup never deduped: the style was re-applied and the guard re-entered on every buffer-monitor tick. The same trap was fixed for `tasAdHidden` in v68.5.6 but missed here. The re-application was accidentally load-bearing — it is what kept an SDA hidden when a React re-render dropped the inline style — so rather than making the hide once-only, the `display:none` re-assert is now deliberate and unconditional (matching the `<video>` guard's documented reasoning) while the marker is assigned `'1'` so it only gates the one-shot debug log. Net effect: the ad stays hidden across re-renders and the log stays quiet. Applies to all four vaft variants.
- **Stream display ads (SDA) were never hidden until the session's first ad break** — `hideTwitchAdOverlays()` opened with an early return on `cachedPlayerRootDiv`, but that cache is only ever assigned inside `updateAdblockBanner()`, which runs on a worker `UpdateAdBlockBanner` message (or a live→VOD transition). On a clean stream neither fires, so the cache stayed `null`, the early return hit on every buffer-monitor tick, and the SDA hide was dead code for the whole pre-break part of the session — the display ad simply sat there. SDAs are a page-level overlay rendered independently of the video player and outside the m3u8, so they never needed a player root: the SDA query already starts at `document`. The guard is removed, so the hide now runs from the first monitor tick regardless of ad state. The `<video>` ad guard sharing the function is unaffected — it keys off `playerForMonitoringBuffering` (optional-chained) and only hides elements whose src is on the Amazon ad CDN, which a `blob:`-fed live player cannot match. No new selectors and no parent walking, per the overlay-hide rule in CLAUDE.md. Applies to all four vaft variants; `strip` and `video-swap-new` have no overlay-hide path.
